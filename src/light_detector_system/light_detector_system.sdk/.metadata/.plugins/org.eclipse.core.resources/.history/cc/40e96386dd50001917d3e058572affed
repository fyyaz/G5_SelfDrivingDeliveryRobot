...
#define MIN_LED_POS 100
#define MAX_LED_POS 600

#define MIN_TOTAL_THRESH 1000

#define MIN_MOTOR_SPEED 0.25
#define MAX_MOTOR_SPEED 0.45

#define MOTOR_FORWARD_DIR 1
#define MOTOR_REVERSE_DIR 0

//Set to 1 to only rotate robot to follow LED
#define FOLLOW_ROTATE 0

//Use these to lock algo in one of these states
//For testing/demoing individual features
#define FOLLOW_ONLY 0
#define AVOID_ONLY 0
...
void update_motor_control(){
	...
	//Check if motors are enabled based on enable switch
	unsigned int motors_enabled = *((unsigned int*)XPAR_AXI_GPIO_0_BASEADDR);
	...
	if(motors_prev_enabled == 0 && motors_enabled == 1){
		HB3_motorEnable(&pmodHB3_L);
		HB3_motorEnable(&pmodHB3_R);
	}
	else if (motors_prev_enabled == 1 && motors_enabled == 0){
		HB3_motorDisable(&pmodHB3_L);
		HB3_motorDisable(&pmodHB3_R);
	}
	else{

	}

	motors_prev_enabled = motors_enabled;
...
void change_drive_state(enum drive_routines state){
	switch(state){
		...
		case SEARCH:
			...
			search_vars.MAX_ROTATE = 100; //Maximum amount of time to rotate for approx 360 degrees
			search_vars.MAX_TRAVEL = 100; //Maximum time to travel
			search_vars.RAND_ROTATE = (int)rand() % search_vars.MAX_ROTATE;
		break;
		case AVOID:
	...
	if (err > 0){
		...
		if (led_found){
			...
			if (search_vars.move_state == 0){
				//Rotate robot
				lm_speed = -(MAX_MOTOR_SPEED + MIN_MOTOR_SPEED)/2.0;
				rm_speed = (MAX_MOTOR_SPEED + MIN_MOTOR_SPEED)/2.0;
				//lm_speed = -MIN_MOTOR_SPEED;
				//rm_speed = MIN_MOTOR_SPEED;
				...
				if (search_vars.travel_ctr == search_vars.MAX_TRAVEL){
					//Done rotating, transition to travelling in a straight line
					search_vars.travel_ctr = 0;
					search_vars.rotate_ctr = 0;
					search_vars.move_state = 0;
...
int get_led_pos(double* led_pos, int* error, int debug_print){
	...
	*error = 0;
	...
	//Map MIN_LED_POS -> -1 MAX_LED_POS -> 1
	double x_loc = 2.0 * (((double)x_pos - MIN_LED_POS) / (MAX_LED_POS - MIN_LED_POS)) - 1.0;
...
void init_sys(){
	...
	*(light_detect_controller + 5) = MIN_TOTAL_THRESH;
...
int main()
{
    ...
    while(1){
    	...
    	unsigned int color = *((unsigned int*)XPAR_COLOR_SELECTOR_GPIO_BASEADDR);
    	if (color != curr_color && color < 3){
    		xil_printf("CHANGED COLOR FROM: %d to %d\n", curr_color, color);
    		*(light_detect_controller + 1) = color;
    		curr_color = color;
    	}
