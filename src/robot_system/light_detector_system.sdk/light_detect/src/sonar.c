...
/*int monitorSonarPmod(int left_right);
...
/*int monitorSonarPmod(int left_right) {
   SonarInitialize(left_right);
   SonarRun();
   SonarCleanup();
   return 0;
}

void SonarInitialize(int left_right) {
   EnableCaches();
   if (left_right == 0) {
      MAXSONAR_begin(&myDevice, PMOD_MAXSONAR_LEFT_BASEADDR, CLK_FREQ);
   } else {
	  //MAXSONAR_begin(&myDevice, PMOD_MAXSONAR_RIGHT_BASEADDR, CLK_FREQ);
   }
}
...
void SonarRun() {
   ...
   while (1) {
      ...
      xil_printf("MAXSONAR: distance (in) = %3d\r", dist);
      usleep(200000);
   }
}
